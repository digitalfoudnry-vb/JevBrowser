...
export function parseSnapshot(text) {
    ...
    for (const line of lines) {
        ...
        if (!trimmed || trimmed.startsWith("[p") || trimmed === "root")
            continue;
 ...
 */
export async function generateStandaloneSnapshot(page) {
    const data = await page.evaluate(() => {
        ...
        for (const el of Array.from(stamped)) {
            ...
            const options = tag === "select"
                ? Array.from(el.options).map(o => o.text.trim()).filter(Boolean).slice(0, 50)
                : undefined;
    ...
    const lines = [`root (viewport snapshot: ${data.length} interactive elements)`];
