...
export function isNoiseHref(href) {
    ...
    if (/^[a-z][a-z0-9+.-]*:/.test(lowered) && !/^https?:/.test(lowered))
        return true;
    if (lowered.startsWith("#") || lowered.startsWith("javascript:") || lowered.startsWith("mailto:"))
        return true;
    return false;
}
export function compileActionSpace(refs) {
    const elements = [];
    const seenHrefs = new Set();
    for (const item of refs) {
        if (elements.length >= MAX_ELEMENTS)
            break;
        if (isNoiseName(item.name))
            continue;
        if (isNoiseHref(item.href))
            continue;
        if (item.href) {
        ...
        if (!item.clickable && !item.typeable && !item.selectable)
            continue;
        const id = `e${elements.length + 1}`;
        const kind = item.typeable ? "type" : item.selectable ? "select" : "click";
...
export function pickAlternate(probabilities, exclude) {
    ...
    for (const [option, p] of ranked) {
        if (option === "back" || exclude.has(option))
            continue;
