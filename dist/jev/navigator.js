...
export async function jevNavigate(task, startUrl, options = {}, existingTaskSpace) {
    ...
    try {
        ...
        try {
            if (options.format === "markdown") {
                const rawHtml = await page.evaluate?.(() => document.body?.innerHTML ?? "").catch(() => "");
        ...
        if (options.screenshot !== "none" && page.screenshot) {
            try {
