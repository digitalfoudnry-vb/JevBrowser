...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (cmd === "run") {
    ...
    let format: "text" | "markdown" = "markdown";
    ...
    for (let i = 0; i < runArgs.length; i++) {
      const arg = runArgs[i];
      if (arg === "--max-steps" && runArgs[i + 1]) maxSteps = Number(runArgs[++i]);
      else if (arg === "--max-seconds" && runArgs[i + 1]) maxSeconds = Number(runArgs[++i]);
      else if (arg === "--allow-typing") allowTyping = true;
      else if (arg === "--format" && runArgs[i + 1]) format = runArgs[++i] as any;
      else if (arg === "--screenshot" && runArgs[i + 1]) screenshotPath = runArgs[++i];
      else if (arg === "--no-screenshot") noScreenshot = true;
    ...
    try {
      ...
      if (screenshotPath && result.screenshot_base64_jpeg) {
        ...
        delete (result as any).screenshot_base64_jpeg;
        (result as any).screenshot_saved_to = screenshotPath;
      }
