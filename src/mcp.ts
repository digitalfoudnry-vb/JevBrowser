...
export async function startMcpServer(version: string = "0.1.0") {
  const server = new McpServer({
    name: "jev-browser",
    version,
  });

  const navigateHandler = async ({ task, start_url, max_steps, max_seconds, allow_typing, format, screenshot }: any) => {
  ...
  const navigateSchema = {
    ...
    description:
      "Autonomously navigate a browser session to achieve a goal. " +
  ...
  const snapshotHandler = async ({ task_space, page }: any) => {
    ...
    return {
      content: [{ type: "text" as const, text: snap.content }],
    };
  };
